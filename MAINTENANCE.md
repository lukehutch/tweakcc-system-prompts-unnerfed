...
- **No catalog — which strings are prompts.** The ~17 MB of unpacked JS holds thousands of string literals, and nothing marks which ones are system prompts vs. error messages, log lines, or UI copy. Anthropic doesn't tag them. `prompts-X.Y.Z.json` is the curated list of *which* literals are the ~525 prompts.
